...
export async function POST(request: Request) {
  ...
  try {
    if (action === "revoke_others" && ctx.sessionId) {
      ...
      await writeAuditLogFromRequest(ctx, request, { action: "revoke_other_sessions", area: "security" });
    } else if (action === "revoke_all") {
      await revokeAllSessions(ctx.userId);
      await writeAuditLogFromRequest(ctx, request, { action: "revoke_all_sessions", area: "security" });
    } else if (action === "revoke_one" && sessionId) {
      await revokeSession(sessionId, ctx.userId);
      await writeAuditLogFromRequest(ctx, request, { action: "revoke_session", area: "security", entityId: sessionId });
    } else {
